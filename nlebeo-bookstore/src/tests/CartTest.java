...
//      quantity as 8 instead of 7
    ...
    public void testClear() {
        ...
        assertTrue("The cart is not empty", tempCartSize > myCart.getCartSize());
    }
    ...
    public void testMemebership() {
        ...
        assertEquals("The total value of the cart is incorrect!", 
            5 * 1.99 + 10 * 2.99 + 8 * 6.99, myCart.calculateTotal().doubleValue(), TOLERANCE);
        
        myCart.setMembership(true);
        
        //if either setMembership() or calculateTotal() malfunction, this one will return false
        assertEquals("The total value of the cart is incorrect!", 
            5 * 1.99 + 2 * 9.99 + 8 * 6.99, myCart.calculateTotal().doubleValue(), TOLERANCE);
    }
    ...
    public void testToString() {
        ...
        final String ERROR_MSG = "Output string does not contain substring!";
            
        assertAll(
            () -> assertTrue(ERROR_MSG, myCart.toString().contains(testString1)),
            
            () -> assertTrue(ERROR_MSG, myCart.toString().contains(testString2)),
            
            () -> assertTrue(ERROR_MSG, myCart.toString().contains(testString3)));
    }
}
