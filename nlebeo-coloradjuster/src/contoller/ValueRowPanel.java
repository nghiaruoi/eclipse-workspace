...
public class ValueRowPanel extends JPanel implements PropertyChangeListener {
    ...
    private final Map<String, Integer> myAllColor = new HashMap<String, Integer>();
    ...
    public ValueRowPanel(final String theColorName, 
                         final int theColorValue) {
        super();
        addColor();
        myColorName = theColorName;
        myAllColor.put(theColorName, theColorValue);
        myColor = new ColorModel(myAllColor.get("Red"), 
                                 myAllColor.get("Green"), 
                                 myAllColor.get("Blue"));
    ...
    private void addColor() {
        myAllColor.put(COLOR_RED, 0);
        myAllColor.put(COLOR_GREEN, 0);
        myAllColor.put(COLOR_BLUE, 0);
    }
    ...
    private void layoutComponents() {
        ...
        myColorDisplayPanel.setBackground(new Color(myAllColor.get(COLOR_RED),
                                                    myAllColor.get(COLOR_GREEN),
                                                    myAllColor.get(COLOR_BLUE)));
        final JLabel rowLabel = new JLabel(myColorName);
        System.out.print(new Color(myAllColor.get(COLOR_RED),
                                   myAllColor.get(COLOR_GREEN),
                                   myAllColor.get(COLOR_BLUE)).getRGB());
        rowLabel.setPreferredSize(LABEL_SIZE);
        myValueField.setText(String.valueOf(myAllColor.get(myColorName)));
        ...
        myValueSlider.setValue(myAllColor.get(myColorName));
    ...
    private void addListeners() {
        ...
        myIncreaseButton.addActionListener(theEvent -> {
            myAllColor.put(myColorName, myAllColor.get(myColorName) + INCREASE_COLOR);
            myColor.setBlue(myAllColor.get(COLOR_BLUE));
            myColor.setGreen(myAllColor.get(COLOR_GREEN));
            myColor.setRed(myAllColor.get(COLOR_RED));
            setIncreaseButton();
        });
    ...
    private void setIncreaseButton() {
        if (myAllColor.get(myColorName) >= ColorModel.MAX_VALUE) {
    ...
    private void setDecreaseButton() {
        if (myAllColor.get(myColorName) <= ColorModel.MIN_VALUE) {
            myDecreaseButton.setEnabled(false);
        } else {
